...
outputSubMeteringPlotPNG <- function(filename, powerData, yLabel) {
  ...
  plot(powerData$powerDates, 
       rep(0, length(powerData$powerDates)), #placeholder data for intial plot frame
       type="n", 
       ylab=yLabel,
       xlab="",
       ylim=c(minMetering, maxMetering))
  
  lines(powerData$powerDates, powerData$Sub_metering_1, col="black")
  lines(powerData$powerDates, powerData$Sub_metering_2, col="dark orange")
  lines(powerData$powerDates, powerData$Sub_metering_3, col="blue")
  
  legend("topright", col=c("black","dark orange","blue"), lty=1, legend= colnames(powerData[,6:8]))
...
outputSubMeteringPlotPNG("plot3", 
                        powerData,
